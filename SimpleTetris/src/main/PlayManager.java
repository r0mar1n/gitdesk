...
public class PlayManager {
	...
	public void update() {
		...
		if(currentMino.active == false) {
			...
			//if the currentMino is active, put it into the staticBlocks
			...
			if(currentMino.b[0].x == MINO_START_X && currentMino.b[0].y == MINO_START_Y) {
				...
				// so it's xy are the name with nextMino's
				gameOver = true;
		...
		while(x < right_x && y < bottom_y) {
			...
			if(x == right_x) {
				...
				if(blockCount == 12) {
					...
					lines++;
					// Drop speed
					// if the line score hits a certain no. increase the drop speed
					//1 is the fastest
